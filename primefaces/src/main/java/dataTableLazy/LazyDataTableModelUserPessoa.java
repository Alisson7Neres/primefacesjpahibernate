...
public class LazyDataTableModelUserPessoa<T> extends LazyDataModel<UsuarioPessoa> {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4665880779188049717L;
	...
	public List<UsuarioPessoa> load(int first, int pageSize, String sortField, SortOrder sortOrder,
		...
		Integer qtdRegistro = Integer.parseInt(dao.getEntityManager().createQuery
				("select count(1)" + getSql()).getSingleResult().toString());
